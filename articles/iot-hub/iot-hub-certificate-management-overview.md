...
| Policy revocation |Revoking a policy removes the associated CA certificate from IoT Hub and adds the policy to the parent CA’s Certificate Revocation List (CRL), thereby blocking all devices from connecting with a certificate issued by that CA. Once the policy is revoked, a new CA certificate is automatically created for that policy and synchronized with IoT Hub.|
