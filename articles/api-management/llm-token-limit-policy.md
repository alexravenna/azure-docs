...
* The value of `remaining-quota-tokens-variable-name` or `remaining-quota-tokens-header-name` is an estimate and may be larger than expected based on actual token consumption. For more information, see [About token counts and estimation](#about-token-counts-and-estimation).
