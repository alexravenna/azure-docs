...
> [!CAUTION]
> Named values referenced in policies are resolved at the service level at runtime. A user with permission to edit policies (for example, write access to Microsoft.ApiManagement/service/apis/policies) can read the contents of any named value by referencing it in a policy using the `{{named-value-id}}` syntax, even if that user doesn't have explicit read access to the named value resource itself (Microsoft.ApiManagement/service/namedValues). When you grant policy editing permissions, consider that this effectively grants read access to all named values in the service instance.
