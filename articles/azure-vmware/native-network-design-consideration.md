...
- /32 route destination from peered VNET: If you are advertising /32 routes from NSX, like HCX MON routes or DNS forwarder routes, and need connectivity to that /32 destination from a peered virtual network, you should create a Support Case through the Azure portal. Connectivity from within the local VNET to the /32 destination functions properly.
