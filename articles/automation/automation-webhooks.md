...
> [!NOTE]
> - Azure Resource Manager (ARM) template outputs are persisted in deployment history and are accessible to any principal with Reader access to the resource group. Please be aware while adding sensitive data (such as webhook URLs or secrets) in outputs, as this may result in unintended exposure.
