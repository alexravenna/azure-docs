...
You can't move a virtual network to a different subscription if the virtual network contains a subnet with resource navigation links. For example, if an Azure Managed Redis resource is deployed into a subnet, that subnet has a resource navigation link.
