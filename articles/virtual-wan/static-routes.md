...
* routing traffic through an Azure Firewall deployed in the Virtual WAN hub
* routing traffic to a designated IP address (often a load balancer in front of a Network Virtual Appliance) deployed in a spoke virtual network that's connected to the Virtual WAN hub.
...
>[!NOTE]
> There are two disjoint ways to direct traffic to Azure Firewall, static routes and [routing intent](how-to-routing-policies.md). Mixing the two configuration options is not supported.

#### Configuration

 Configure Virtual WAN to route traffic to Azure Firewall in a secure hub using static routes. This configuration involves adding two separate configurations to your Virtual WAN deployment:
...
| Use case | Alternative approach |
|--|--|
|Double-inspection scenarios: inspect traffic destined for indirect spoke or Internet with Azure Firewall deploye in a secure hub. Then, forward traffic to NVA in spoke for breakout or access to an indirect spoke.| Use [routing intent and policies](how-to-routing-policies.md) and static routes on Virtual Networks connection with **Propagate static routes** set to **true**.|