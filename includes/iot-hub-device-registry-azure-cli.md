...
- The Azure IoT CLI extension with previews enabled installed to access the ADR integration and certificate management functionalities for Azure IoT Hub:
...
1. Create a DPS with a linked IoT hub and namespace.
...
1. Retrieve the principal ID of the UAMI. This ID is needed to assign roles to the identity.
...
1. Create a new IoT hub that's linked to the ADR namespace and with the UAMI that you created earlier.
...
1. Assign the Contributor role to the ADR identity. This role grants the ADR namespace's managed identity Contributor access to the IoT hub. The role allows broad access, including managing resources, but not assigning roles.

    ```azurecli-interactive
    az role assignment create --assignee $ADR_PRINCIPAL_ID --role "Contributor" --scope $HUB_RESOURCE_ID
    ```

1. Assign the IoT Hub Registry Contributor role to the ADR identity. This role grants more specific permissions to manage device identities in the IoT hub. These permissions are essential for ADR to register and manage devices in the hub.
...
1. Create a new DPS instance linked to your ADR namespace that you created in the previous sections. Your DPS instance must be located in the same region as your ADR namespace.
...
The following commands help you manage your ADR namespaces, disable devices, create custom policies, and delete resources when they're no longer needed.
...
To delete your ADR namespace, you must first delete any IoT hubs that are linked to the namespace.
